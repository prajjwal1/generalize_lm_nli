...
class SiameseTransformer(nn.Module):
    def __init__(self, args, config):
        ...
        #self.model_b = AutoModelForSequenceClassification.from_pretrained(self.args.model_name,
        #                   config=config, cache_dir=self.args.cache_dir)
    ...
    def forward(self, input_a, input_b):
        ...
        input_a.pop('labels')
        input_b.pop('labels')
        output_a = self.model_a(**input_a)[0]
        output_b = self.model_a(**input_b)[0]
        
        # cls_tokens_a = output_a[:, 0, :]
        # cls_tokens_b = output_b[:, 0, :]
