...
def siamese_hans_convert_examples_to_features(
    ...
    for (ex_index, example) in tqdm.tqdm(
        ...
        features_a.append(InputFeatures(**inputs_a, label=label, pairID=pairID))
        features_b.append(InputFeatures(**inputs_b, label=label, pairID=pairID))

    for i, example in enumerate(examples[:5]):
        logger.info("*** Example ***")
        logger.info(f"guid: {example}")
        logger.info(f"features: {features_a[i]}")
...
class SiameseHansDataset(Dataset):
    """
        ...
        with FileLock(lock_path):
            ...
            if os.path.exists(cached_features_file) and not overwrite_cache:
                ...
                (
                    self.features_a,
                    self.features_b,
                ) = siamese_hans_convert_examples_to_features(
...
def siamese_hans_data_collator(batch):
    features_a, features_b = [], []
    for item in batch:
    ...
    batch_a = default_data_collator(features_a)
    _ = batch_a.pop("pairID", None)

    batch_b = default_data_collator(features_b)
    _ = batch_b.pop("pairID", None)
